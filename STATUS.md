...
- [x] GeoTIFF harness: tiepoint (`(minX, maxY)`) and pixel-scale (`span/(n−1)`) values are decoded and asserted (`Tools/ViewerHarness/main.swift:1877-1886`), and so are the GeoKeyDirectory (tag 34735) header and its three keys read from the file: version 1 / revision 1.0 / 3 keys, GTModelType=1 (Projected), GTRasterType=2 (PixelIsPoint), ProjectedCSType=3857 (`main.swift:1828-1848`, commit `efbd644`, verified 2026-09-18).
- [x] GPU/lease harness: non-square grid (200×120) run through both `leasedReliefProducts` and `reliefProducts`, slope checked against CPU reference for both (`main.swift:2069-2122`, verified 2026-09-18).
- [x] Lease harness: pool-reuse-while-reading forced — a held lease's snapshot verified unchanged across 10 churn dispatches of other grid sizes (`main.swift:2124-2168`, verified 2026-09-18).
- [x] `GeoTileKey`: all four globe corners (±90 lat, ±180 lon) plus over-range clamping and Morton locality asserted (`main.swift:2170-2210`, verified 2026-09-18).
...
- [x] `GeoTileKey` identity decided (2026-09-18): **south-west origin plus zoom; span is deliberately not part of the key.** Exact for a tile's own (unpadded) extent at every zoom the app serves (up to 21), where origin and zoom fix the extent; any other region must key on `GeoRegion.cacheKey`, which encodes all four bounds (to 1e-6 degrees). Folding span in was rejected: all 64 bits are used (6 zoom + 29 + 29), it would still not make this a grid key (`pixels`/`margin`/`targetSamples` sit outside it), and only the harness constructs a `GeoTileKey` — the sole other references are the uncalled `TileDiskCache.read(for:)`/`map(for:)`/`write(_:for:)` overloads that accept one. Recorded in the type doc (`GeoRegion.swift:262-270`) and pinned in the harness (`main.swift:1702-1775`): span-blindness across four span variants, a per-bound check (moving only the north or east bound leaves the key unchanged), and two checks on real geometry through `TerrainTileOverlay.region(for:)` (a tile and its south-west child share origin bits, and 5×5 neighbourhoods at z21 at the equator, 39°N and 70°N key uniquely). A mutant that folds span into the key is caught by four checks: those three pins and the existing `legacyCacheKey` check. The same edit corrected the doc's quantum (about 3.7 cm latitude / 7.5 cm longitude, not 0.34 m) and dropped its "collision-free" overclaim.
- [ ] Remove `GeoTileKey.legacyCacheKey` and the `TileDiskCache` zoom-0 fallback (review item D4, `docs/superpowers/reviews/2026-09-13-assessment/drafts/leases-histogram-analyst.md`). Because `zoom` defaults to 0, an entry stored at zoom 0 answers a read at *any* zoom at the same origin, so zoom-0 entries act as origin-only wildcards at the cache layer (entries at other zooms do not). The string-keyed `read(forKey:)`/`map(forKey:)` that production uses have no such fallback. Dead code today, so no user-visible effect. Doing it means making `zoom` required and rewriting the `legacyCacheKey` check at `main.swift:1698`; the note documenting the fallback sits on `legacyCacheKey`'s own doc (`GeoRegion.swift:287-290`), so it goes with the property. Not started.
