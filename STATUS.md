...
- `Domain/TransectExporter.swift`: GeoJSON (RFC 7946: a 3D profile LineString plus a Point per detected earthwork signature) and CSV (RFC 4180) export of a transect analysis, georeferenced through the `GeoreferencedElevationField` the transect was sampled from. Library only: no UI entry point yet.
...
- `TerrainTileProvider.analyticalRaster(for:product:options:destinationSize:)` (`MapLayer/TerrainTileOverlay.swift`): any micro-topography product over a region as a node-registered float `ElevationGrid` that `GeoTIFFWriter` exports unchanged. The mosaic is padded by the product's neighbourhood radius and only the requested window is rendered; cells are ≥ 1 m and coarsen to fit `destinationSize`. Reads the in-memory tile cache only; `.relativeElevation` returns nil (it needs a thalweg). Library only: no UI entry point yet.
...
- [x] Elevation GeoTIFF export registration (`TerrainTileProvider.activeGrid(covering:)`, 2026-09-20): the grid was labelled with the mosaic's outer bounds, but `ElevationGrid` and `GeoTIFFWriter` are node-registered (`PixelIsPoint`), so every cell read n/(n-1) too large (1.0152 m instead of 1.00 m at 60 columns) and samples sat up to half a cell off, growing from the centre to the perimeter. It now spans the cell centres, as `analyticalRaster` does. GeoTIFFs exported before this fix carry that error. Harness C10 (`ProviderMicroChecks.swift`).
